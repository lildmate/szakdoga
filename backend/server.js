...
import { db, transporter } from './endpoints/config.js';
import adminRoutes from './endpoints/admin.js';
...
app.get("/getOrder", async (req, res) => {
    ...
    if (!token) {
        ...
        if (user.length === 0) {
            return res.json({ message: "Felhasználó nem található." });
        }
        ...
        if (orders.length === 0) {
            return res.json({ message: "Nem található rendelés." });
        }
        res.json(orders);
    } catch (err) {
